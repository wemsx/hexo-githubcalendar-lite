    ...
    var github_canlendar = (git_user, git_color) => {
      ...
      var responsiveChart = () => {
        if (document.getElementById("gitcanvas")) {
          var git_tooltip_container = document.getElementById(
          ...
          width = github_calendar_c.width = document.getElementById(
            "gitcalendarcanvasbox"
          ).offsetWidth;
          height = github_calendar_c.height =
            (9 * 0.96 * github_calendar_c.width) / git_data.length;
          ...
          for (var week in git_data) {
            ...
            for (var day in weekdata) {
              ...
              github_calendar_ctx.fillStyle = git_thiscolor(
                git_color,
                weekdata[day].count
              );
          ...
          if (document.body.clientWidth > 700) {
            ...
            for (var index in git_monthchange) {
              github_calendar_ctx.fillText(
                git_monthchange[index],
                monthindexlist,
                0.7 * linemaxwitdh
              );
      ...
      const getlist = (target, reg) => {
        let result = [...target.matchAll(reg)];
        let arr = [];
        for (let item of result) {
          let data = item[1] == "No" ? "0" : item[1];
          arr.push(data);
        }
        return arr;
      };
      ...
      requestData(`https://github.com/${git_user}`)
        .then((target) => {
          ...
          let list_count = getlist(target, regex_count);
          let list_date = getlist(target, regex_date);
          list_count = list_count.map(Number);
          let list = [];

          for (let key in list_date) {
            list.push({ date: list_date[key], count: list_count[key] });
          }

          let data = {
            total: sum(list_count),
            contributions: reformList(list, 7),
          };
          ...
          git_montharrbefore = git_month.splice(
            git_monthindex,
            12 - git_monthindex
          );
          ...
          addweek(data);
          addlastmonth();
          var html = github_main_box(
            git_monthchange,
            git_data,
            git_user,
            git_color,
            git_total,
            git_thisweekdatacore,
            git_weekdatacore,
            git_oneyearbeforeday,
            git_thisday,
            git_aweekago,
            git_amonthago
          );
      ...
      var tooltip_html = (x, y, span1, span2) => {
        var html = "";
        html +=
          '<div class="gitmessage" style="top:' +
          y +
          "px;left:" +
          x +
          'px;position: fixed;z-index:9999"><div class="angle-wrapper" style="display:block;"><span>' +
          span1 +
          "&nbsp;</span><span>" +
          span2 +
          " 次上传</span></div></div>";
      ...
      var github_canvas_box = () => {
        var html =
          '<div id="gitcalendarcanvasbox"> <canvas id="gitcanvas" style="animation: none;"></canvas></div>';
        return html;
      };
      var github_info_box = (user, color) => {
        var html = "";
        html +=
          '<div id="git_tooltip_container"></div><div class="contrib-footer clearfix mt-1 mx-3 px-3 pb-1"><div class="float-left text-gray">数据来源 <a href="https://github.com/' +
          user +
          '" target="blank">@' +
          user +
          '</a></div><div class="contrib-legend text-gray">Less <ul class="legend"><li style="background-color:' +
          color[0] +
      ...
      var github_main_box = (
        ...
        html +=
          '<div class="position-relative"><div class="border py-2 graph-before-activity-overview"><div class="js-gitcalendar-graph mx-md-2 mx-3 d-flex flex-column flex-items-end flex-xl-items-center overflow-hidden pt-1 is-graph-loading graph-canvas gitcalendar-graph height-full text-center">' +
          canvasbox +
          "</div>" +
          infobox +
          "</div></div>";
        html +=
          '<div style="display:flex;width:100%"><div class="contrib-column contrib-column-first table-column"><span class="text-muted">过去一年提交</span><span class="contrib-number">' +
          total +
          '</span><span class="text-muted">' +
          oneyearbeforeday +
          "&nbsp;-&nbsp;" +
          thisday +
          '</span></div><div class="contrib-column table-column"><span class="text-muted">最近一月提交</span><span class="contrib-number">' +
          thisweekdatacore +
          '</span><span class="text-muted">' +
          amonthago +
          "&nbsp;-&nbsp;" +
          thisday +
          '</span></div><div class="contrib-column table-column"><span class="text-muted">最近一周提交</span><span class="contrib-number">' +
          weekdatacore +
          '</span><span class="text-muted">' +
          aweekago +
          "&nbsp;-&nbsp;" +
